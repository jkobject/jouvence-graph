...
# `edges__cell_line_gene_essentiality`

- Layer: `edges`
- Status: **canonical present**
- As of: `2026-07-27`
- Canonical URI/pattern: `gs://jouvencekb/main/edges/cell_line_gene_essentiality.parquet`
...
|---|---:|---:|---|---|---|
| `gs://jouvencekb/main/edges/cell_line_gene_essentiality.parquet` | 1,433,992 | 1,172,674 | `1785155481243885` | `rU2CCQ==` | `0M+JHnfjagPyMsUIuAYIbA==` |
...
LOCAL_DIR='./parquet-catalog-data/edges__cell_line_gene_essentiality'
rm -rf -- "$LOCAL_DIR"
mkdir -p "$LOCAL_DIR"
gcloud storage cp --billing-project="$BILLING_PROJECT" 'gs://jouvencekb/main/edges/cell_line_gene_essentiality.parquet' "$LOCAL_DIR/"
```
...
paths = sorted(fs.glob('jouvencekb/main/edges/cell_line_gene_essentiality.parquet'))
...
SELECT "x_id" FROM read_parquet('./parquet-catalog-data/edges__cell_line_gene_essentiality/cell_line_gene_essentiality.parquet') ORDER BY "x_id" NULLS LAST LIMIT 5;
```
